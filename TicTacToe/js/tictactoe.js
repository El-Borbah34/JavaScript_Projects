...
//each player and determine which player won the dice roll.
function rollForTurn()  {
    ...
    // build the sting to show which player rolled what die roll
    for (i=0; i<xArray.length;i++)  {
        var result = i +1;
        var pOne = xArray[0];
        var pTwo =xArray[1];
        if (pOne == pTwo)   { //rigging roll on tie to aviod bug in code. Need to address this later...
            pOne = 1;
            pTwo = 2;
        }
...
function startGame()    {
    ...
    btnDisabled(btn);   //disable the start button now that the game is afoot
    var btn = document.getElementById("btnStop");
    stopEnabled(btn); //enable the stop button now that the game is afoot
...
function stopGame()     {
    ...
    var btn = document.getElementById("btnStop");
    btnDisabled(btn); //disabled the stop button since the game is now stopped
    var showPlayer = document.getElementById("showPlayer");
    ...
    // this clears the running log of all the game moves
...
// this function will conceal the message from console view
...
function determineAvatar()  {
    // determine the correcrt avatar to paint for the active player
    ...
    return paintAvatar; // returned back to the correct avatar
}
...
function recordMoves(square)    {
    ...
    verdict = check(info,square); //call functions to check if proposed square is already occupied
    return verdict;
}
...
function checkForWinCon()   {
    ...
    // call this following array of functions to check for any of the possible wins cons
    ...
    //check console.log (New Check: "+document.getElementById("gameMsg").innerHTML");
    check4Tie();
}
...
function glowBoard(pos)     {
    ...
    for (var i=0;i<squares.length;i++)  {
        if (i == index0)    {
            ...
            setTimeout(function() {bg1.style.backgroundColor= "rgb (66, 244, 266)";}, 1000);
            ...
            setTimeout(function() {bg2.style.backgroundColor= "rgb (264, 238, 66)";}, 400);
            ...
            setTimeout(function() {bg2.style.backgroundColor= "rgb (264, 238, 66)";}, 900);
...
function squareSound()  {
    ...
    sound.play();
    setTimeout(function() {sound.pause();}, 400); //add delays to these to keep sound short
...
function blink()    {
    ...
    setTimeout(function() {body.style.backgroundColor = "#f794f7";}, 700);
...
function checkWinCon1(info,squareArray)  {
    ...
    //gametimes searching for the existence of 
    ...
    //this will trigger (ONLY) if there was a match for index0, index1, index2
...
function checkWinCon2(info,squareArray)  {
    ...
    if (match3Avatar != undefined && match4Avatar != undefined && match5Avatar != undefined)    {
        if (match3Avatar == match4Avatar && match3Avatar == match5Avatar)   {
            ...
            return; ***DONT KNOW IF I NEED its in the function above*/
        }
    }
...
function checkWinCon5(info,squareArray)  {
    ...
    winner(winDetected,winCon5); //winCon4 is the array of win combo
}
...
function square1Animate()   {
    ...
    if (activePlayer != "Game Stopped") { //if game has not yet started prevent avatar placement
        ...
        if (verdict == undefined)   { //if verdict is empty then the square is unoccupied.
            ...
            checkForWinCon(); // call functions to check if current move completes a winning condition.
            avatarPlaced(square,paintAvatar); // end current turn and pass the turn to the other player
            squareSound(); // play a game sound when the avatar i placed.
        }
    }
}
